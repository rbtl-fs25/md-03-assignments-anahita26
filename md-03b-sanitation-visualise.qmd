...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    The order of the bars in the plot matches the order of the legend. The order they appear in is most likely due to the order they appear in the data set.

2.  What would you want to change?

    I would like the legend to move in order of worst to best sanitation services. E.g. starting with no sanitation facilities and moving upwards.

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?

    This allows us to observe only the countries that do not have sufficient sanitation services.
...
2.  What do you observe when you look at the code and plot?
    ...
    The facet_grid function first creates subplots based on each sanitation category and residence type. The second argument of the facet_grid function allows for a different scaling of the y-axis so we can observe general trends in the data. The colors in the graph are based on each sanitation category; however, the legend was removed.
